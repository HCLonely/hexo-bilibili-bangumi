...
module.exports = function () {
  var _ref = (0, _asyncToGenerator2["default"])(_regenerator["default"].mark(function _callee(locals) {
    var _config$type, _config$type$lazyload, _config$type$source, _config$type$showMyCo, _config$type$paginati, _config$type$progress, _config$type$swup, _config$type2;
    var type,
      config,
      full_url_for,
      root,
    ...
    return _regenerator["default"].wrap(function (_context) {
      while (1) switch (_context.prev = _context.next) {
        ...
        case 1:
          ...
          pageTheme = ['light', 'dark', 'auto'].includes(String(config[type].theme).toLowerCase()) ? String(config[type].theme).toLowerCase() : 'auto';
          _context.next = 2;
          return pug.renderFile(path.join(__dirname, 'templates/bangumi.pug'), {
            ...
            swupCompatible: (_config$type$swup = config[type].swup) !== null && _config$type$swup !== void 0 ? _config$type$swup : false,
            theme: fs.existsSync(path.join(__dirname, "templates/theme/".concat(config.theme, ".min.css"))) ? config.theme : null,
            pageTheme: pageTheme,
            pugTemplate: config[type].pagination ? fs.readFileSync(path.join(__dirname, "templates/".concat(TEMPLATE_MAP[config[type].source]))).toString().replace('.bangumi-item', '.bangumi-item.bangumi-hide').replace(/__\('(.+?)'\)/g, function (match, key) {
