...
module.exports = async function (locals, type = 'bangumi') {
  ...
  if (!fs.existsSync(path.join(this.source_dir, `/_data/${type}s.json`))) {
    ...
    ({
      ...
      watching,
      watched
    } = JSON.parse(fs.readFileSync(path.join(this.source_dir, `/_data/${type}s.json`))));
    if (fs.existsSync(path.join(this.source_dir, `/_data/extra_${type}s.json`))) {
      ...
      const {
        ...
        watchingExtra,
        watchedExtra
      } = JSON.parse(fs.readFileSync(path.join(this.source_dir, `/_data/extra_${type}s.json`)));
      if (wantWatchExtra) {
  ...
  const pageTheme = ['light', 'dark', 'auto'].includes(String(config[type].theme).toLowerCase()) ? String(config[type].theme).toLowerCase() : 'auto';
  const contents = await pug.renderFile(path.join(__dirname, 'templates/bangumi.pug'), {
    ...
    pageTheme,
    pugTemplate: config[type].pagination ? fs.readFileSync(path.join(__dirname, `templates/${TEMPLATE_MAP[config[type].source]}`)).toString().replace('.bangumi-item', '.bangumi-item.bangumi-hide').replace(/__\('(.+?)'\)/g, (match, key) => `('${__(key)}')`) : '',
